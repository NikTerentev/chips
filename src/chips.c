...
SDL_AppResult SDL_AppInit(void** appstate, int argc, char* argv[]) {
    ...
    *appstate = as;
    ...
    SDL_asprintf(
        &wav_path, "%sassets/441634__xtrgamr__asynth.wav", SDL_GetBasePath()
    );
    ...
    // Create audio stream
    as->stream = SDL_OpenAudioDeviceStream(
    ...
    if (!SDL_CreateWindowAndRenderer(
            "examples/emulator/chip-8", SDL_WINDOW_WIDTH, SDL_WINDOW_HEIGHT, 0,
...
void decode_instruction(
    uint16_t instruction, char** message, AppState* appstate
) {
    ...
    case 0x0:
        ...
        case 0x0:
            SDL_memset(
                appstate->chip8_context.display_cells, 0,
    ...
    case 0x1:
        ...
        snprintf(
            *message, 256, "Jump to address %x", second_third_and_fourth_nibbles
        );
        break;
    case 0x2:
        ...
        appstate->chip8_context.PC = second_third_and_fourth_nibbles;
        snprintf(
            *message, 256, "Call the subroutine at memory location %x",
    ...
    case 0x3:
        ...
        snprintf(
            *message, 256, "Skip next instruction if V%x is equal to %x",
    ...
    case 0x4:
        ...
        snprintf(
            *message, 256, "Skip next instruction if V%x is not equal to %x",
            second_nibble, third_and_fourth_nibbles
        );
        break;
    case 0x5:
        ...
        snprintf(
            *message, 256, "Skip next instruction if V%x is equal to V%x",
    ...
    case 0x6:
        ...
        snprintf(
            *message, 256, "Set value %x to register V%x",
    ...
    case 0x7:
        appstate->chip8_context.V[second_nibble] += third_and_fourth_nibbles;
        snprintf(
            *message, 256, "Add value %x to register V%x",
            third_and_fourth_nibbles, second_nibble
        );
        break;
    case 0x8:
        ...
        case 0x0:
            appstate->chip8_context.V[second_nibble] =
                appstate->chip8_context.V[third_nibble];
            snprintf(
                *message, 256, "V%x is set to the value of V%x", second_nibble,
                third_nibble
            );
            break;
        case 0x1:
            ...
            snprintf(
                *message, 256, "V%x is set to the bitwise OR of V%x and V%x",
        ...
        case 0x2:
            ...
            appstate->chip8_context.V[15] = 0;
            snprintf(
                *message, 256, "V%x is set to the bitwise AND of V%x and V%x",
                second_nibble, second_nibble, third_nibble
            );
            break;
        case 0x3:
            ...
            appstate->chip8_context.V[15] = 0;
            snprintf(
                *message, 256, "V%x is set to the bitwise XOR of V%x and V%x",
                second_nibble, second_nibble, third_nibble
            );
            break;
        case 0x4:
        ...
        case 0x5:
            ...
            if (is_lower)
                ...
                appstate->chip8_context.V[15] = 0;
            snprintf(
                *message, 256, "V%x is set to the value of V%x - V%x",
                second_nibble, second_nibble, third_nibble
            );
            break;
        case 0x6:
    ...
    case 0x9:
        ...
        snprintf(
            *message, 256, "Skip next instruction if V%x is not equal to V%x",
            second_nibble, third_nibble
        );
        break;
    case 0xA:
        ...
        snprintf(
            *message, 256, "Set address %x to register I",
            second_third_and_fourth_nibbles
        );
        break;
    case 0xB:
        appstate->chip8_context.PC = second_third_and_fourth_nibbles +
                                     appstate->chip8_context.V[second_nibble];
        snprintf(
            *message, 256, "Jump to address %x + value from V%x",
    ...
    case 0xF:
        ...
        case 0x0A:
            ...
            for (size_t key_index = 0; key_index < 16; key_index++) {
                ...
                if (appstate->keyboard_state[cur_scancode]) {
                    snprintf(
                        *message, 256,
                        "Key %zx is pressed, number putted in V%x.", key_index,
                        second_nibble
                    );
                    SDL_ResetKeyboard();
                    appstate->chip8_context.V[second_nibble] =
            ...
            if (!is_key_pressed) {
                ...
                snprintf(
                    *message, 256,
                    "Wait for key input, put key number into V%x", second_nibble
                );
                break;
            }
            break;
        case 0x29:
            ...
            snprintf(
                *message, 256, "Point I to the %x character", second_nibble
            );
            break;
        case 0x33:
        ...
        for (i = 0; i < CHIP8_DISPLAY_WIDTH; i++) {
            ...
            if (display_cell == 0) {
                SDL_SetRenderDrawColor(
                    appstate->renderer, 0, 0, 0, SDL_ALPHA_OPAQUE
                );
            } else {
                SDL_SetRenderDrawColor(
                    appstate->renderer, 255, 255, 255, SDL_ALPHA_OPAQUE
                );
            }
            set_rect_xy_(&r, i, j);
        ...
        if (as->chip8_context.sound_timer > 0) {
            ...
            if (SDL_GetAudioStreamQueued(as->stream) < (int)as->wav_data_len) {
                SDL_PutAudioStreamData(
                    as->stream, as->wav_data, as->wav_data_len
                );
            }
